...
def draw(best_route, best_dist, x, y, iteration):
    global canv,lx,ly
    canv.clear()
    ...
    canv.plot([x[best_route[-1]], x[best_route[0]]],[y[best_route[-1]], \
      y[best_route[0]]],'k')
    canv.plot(x,y,'ro')
    title("Distance : %f   Iteration: %d" % (best_dist, iteration))
    pause(TPSPause)
...
for i in range(Nb_cycles):
    ...
    if i % 10 == 0:
        #draw(swarm,i)
        draw(best['bestpos'], best['bestfit'], x, y, i)
        print("Swarm: \n")
        for e in swarm:
            print(e['pos'])
...
draw(best['bestpos'], best['bestfit'], x, y, Nb_cycles)